...
feature 'As a user who wants to add content to the site' do
  scenario 'I can sign up for an individual account', js:true do
    ...
    user = User.find_by_email('jimmy@example.com')
    expect(user.first_name).to eql('Jimmy')
    expect(user.last_name).to eql('Tables')
    expect(user.screen_name).to eql('Jim')
    expect(user.email).to eql('jimmy@example.com')
  ...
  context 'with an existing user' do
    ...
    scenario "I can sign in to my account" do
      ...
      expect(current_path).to eql(my_getting_started_path)
