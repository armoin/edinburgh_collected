  ...
  def unmoderate
    ...
    respond_to do |format|
      if memory.unmoderate!
        format.html { redirect_to admin_unmoderated_path, notice: 'Memory unmoderated' }
        format.json { render json: memory }
      else
        format.html { redirect_to admin_unmoderated_path, alert: 'Could not unmoderate memory' }
